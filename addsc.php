...
<html>
	...
	<body>
		...
		<div class="main">
			...
			<?php
			if(!($stmt = $mysqli->prepare("INSERT INTO Students_classes (Student_ID, Class_ID, Grade) VALUES (?,?,?)"))){
				echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!($stmt->bind_param("iis",$_POST['Student_ID'],$_POST['Class_ID'],$_POST['Grade']))){
				echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!$stmt->execute()){
				echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
			} else {
				echo "Success! Added " . $stmt->affected_rows . " row to Students-Classes Enrollment.";
			}
			?>
		</div>
		</div>
