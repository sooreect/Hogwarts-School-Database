...
<html>
	...
	<body>
		...
		<div class="main">
			...
			<?php
			if(!($stmt = $mysqli->prepare("INSERT INTO Professors (FirstName, LastName, House_ID) VALUES (?,?,?)"))){
				echo "Prepare failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!($stmt->bind_param("ssi",$_POST['FirstName'],$_POST['LastName'],$_POST['House_ID']))){
				echo "Bind failed: "  . $stmt->errno . " " . $stmt->error;
			}
			if(!$stmt->execute()){
				echo "Execute failed: "  . $stmt->errno . " " . $stmt->error;
			} else {
				echo "Success! Added " . $stmt->affected_rows . " row to Professors.";
			}
			?>
		</div>
	</body>
